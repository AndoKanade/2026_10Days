...
void Obj3dCommon::CreateRootSignature(){
	...
	D3D12_ROOT_PARAMETER rootParameters[9] = {}; // 8つに増やす
	...
	rootParameters[8].ParameterType = D3D12_ROOT_PARAMETER_TYPE_SRV; // StructuredBufferの直接渡し
	rootParameters[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // 頂点シェーダーで使う
	rootParameters[8].Descriptor.ShaderRegister = 2; // t2レジスタ
...
void Obj3dCommon::CreateGraphicsPipelineState(){
	...
	// スキニングモデル用のシェーダー読み込み (専用の頂点シェーダー)
	...
	// スキニングモデル用の頂点レイアウト (WEIGHTとINDEXを追加)
	...
	// 通常のPSO設定をコピーして、レイアウトと頂点シェーダーだけ差し替える
