...
未実装の主な項目：スコア計算・表示、ゲーム画面上のUI（現状はImGuiのみ）、タイトル／リザルトの体裁、BGM再生とSE、明示的な状態機械への整理、天井警告や消去エフェクトなどの演出、盤面幅 6/10 の切り替えUI。詳細と着手順は「今後の予定」の「残っている作業」を参照。
...
5. **BGMとSE。** `SoundLoadFile(kBgmPath_)` を呼ぶだけで再生していない。SE（設置・回転・通電・消去・連鎖・ゲームオーバー）は未作成。**未着手。**
