...
namespace PuzzleConfig{
	...
	// 追加：消去演出中、光の波が届いて弾ける直前のマスをどれだけ膨らませるか。
	// 元の拡大率に対する倍率の増分。0.5なら最大で1.5倍まで膨らむ。
	constexpr float kClearPopScaleAmount = 0.5f;
	...
	// --- 追加：壁ブロック（ゴール・電源）の質感 ---
	// 壁も落下ブロックと同じ面取りキューブで描画し、同じようにツヤを出す。
	// ただし壁のテクスチャは黒がベースで、拡散光は元の色に掛け算されるため黒のままになる。
	// 黒い面をテカらせるのは加算される鏡面反射と映り込みなので、ブロックより強めに設定する。

	// 鏡面反射の鋭さ。ブロックより小さくしてハイライトを広げ、
	// 面取り面が白く光って見えるようにする。
	constexpr float kWallShininess = 12.0f;

	// 環境マップ（スカイボックス）の映り込みの強さ。
	// 黒い面に色が乗る唯一の要素なので、ブロックより強くして濡れたような黒にする。
	constexpr float kWallEnvironmentCoefficient = 0.05f;
